...
def iou( dt, gt, pyiscrowd ):
    ...
    # cdef 定义cpython np.ndarray 类型的静态变量
    cdef np.ndarray[np.uint8_t, ndim=1] iscrowd = np.array(pyiscrowd, dtype=np.uint8)
    # simple type checking
    # siz: unsigned long type
    cdef siz m, n
    # 如果为bbox 类型，只是返回numpy 化的原来结果，如果为 rle(for segmentation) 有别的处理
    dt = _preproc(dt)
    gt = _preproc(gt)
    m = _len(dt)
    n = _len(gt)
    if m == 0 or n == 0:
        return [] # 如果有其中一个m,n 为0，返回空
    ...
    # 开辟存储iou的空间
    # mxn,应该是保存每个dt 的box 和 每个 gt 的box的iou
    _iou = <double*> malloc(m*n* sizeof(double))
    iou = np.zeros((m*n, ), dtype=np.double)# 这个意义在哪里，被下面的iou 重写了
    shape[0] = <np.npy_intp> m*n
    #此处iou 只是一片连续内存
    #在实现时，当作形状为(n,m) 的格式填充
...
""" bbIou： 核心：两个bbox, 选右下角小的x,y(找iou右下角), 选左上角大的x,y(找iou右上角), 前者-后者
                # 或者理解为iou右边由两个最右边小的决定，iou上边由两个上边大的决定。
#BB: double*  *o 是一个指向n*m double 类型的内存的指针
void bbIou( BB dt, BB gt, siz m, siz n, byte *iscrowd, double *o ) {
  double h, w, i, u, ga, da; siz g, d; int crowd;
  # 遍历gt
  for( g=0; g<n; g++ ) {
    # G 用来定位gt 的位置，*4 因为每次遍历4个元素(bbox)
    # ga： 计算bbox 的area, crowd:大部分为0
    BB G=gt+g*4; ga=G[2]*G[3]; crowd=iscrowd!=NULL && iscrowd[g];
    # 遍历dt
    for( d=0; d<m; d++ ) {
      # o[g*m+d] 对应于数组o[g][d],g行d列，也就是把o看成 o[n][m] 的形状
      BB D=dt+d*4; da=D[2]*D[3]; o[g*m+d]=0;
      # fmin,fmax c语言中快速比较两个元素的大小关系
      # D[0,3]=[x1,y1,w,h]
      # fmin(dx2,gx2) # 选右下角小的x - fmax(dx1,gx1) # 左上角大的x
      # 或者理解为iou右边由两个最右边小的决定，iou上边由两个上边大的决定。
      # 如果<=0，就是没有交集
      w=fmin(D[2]+D[0],G[2]+G[0])-fmax(D[0],G[0]); if(w<=0) continue;
      # fmin(dy2, gy2) # 选右下角小的y - fmax(dy2, gy2) # 右上角的大
