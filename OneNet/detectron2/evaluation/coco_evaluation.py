...
# DatasetEvaluator 需要实现三个方法： reset(), process(inputs, ouputs), evaluate()
class COCOEvaluator(DatasetEvaluator):
    ...
    def reset(self):
        # 存储的内容格式
        """
        item in self._predictions(dict): {
            "image_id": 图片id号，用于标识图片的，coco_api可据此获取对应的annotation 信息
            "instances":{
                "image_id": img_id
                "category_id": classes[k], # 预测的类别id
                "bbox": boxes[k], # 预测的box(x,y,w,h)
                "score": scores[k], # 预测分数
            }
        }
        """
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            if "instances" in output:
                ...
                # in current file
                """ return of instances_to_coco_json
                list[result]
                result = {
                "image_id": img_id, 图片id
                "category_id": classes, # 预测的类别id
                "bbox": [4], ->(x,y,w,h)# 预测的box
                "score": score, # 预测分数
                }"""
                # 主要是把 bbox 的模式从xyxy -> (x,y,w,h)
    ...
    def _tasks_from_predictions(self, predictions):
        ...
        for pred in predictions:
            ...
            if "keypoints" in pred:
                tasks.add("keypoints")
        return sorted(tasks)
    # 负责转化catgory_id, dumps json 文件保存，转向使用COCO_api 执行eval
...
"""
coco_gt: coco_api
coco_results: list[dict:{"image_id":, category_id":, "bboxe":, "score":}]
iou_type: bbox, for detection
"""
def _evaluate_predictions_on_coco(
    ...
    # return: 返回的是把我们预测的结果适配于COCO 的 new coco_api
    coco_dt = coco_gt.loadRes(coco_results)
    # defualt: use_fast_impl
    # set use_fast_impl to False,我看COCOeval
