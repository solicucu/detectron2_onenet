...
class ImageList(object):
    ...
    def __init__(self, tensor: torch.Tensor, image_sizes: List[Tuple[int, int]]):
        ...
        """
        self.tensor = tensor
        self.image_sizes = image_sizes # image sizes after transformation
    ...
    def from_tensors(
        ...
        if len(tensors) == 1:
            ...
            # unsquueze_ 转为一个batch
            #                            dim=-1,    dim=-2,      dim=-3
            # F.pad() pading_size = (left, right, top, bottom,front, back)
            # 显然只能在右下方padding,因为左上角是annotation的原点坐标
            batched_imgs = F.pad(tensors[0], padding_size, value=pad_value).unsqueeze_(0)
        else:
            # max_size can be a tensor in tracing mode, therefore convert to list
            # [batch_size] + [channel] + [max_size]
            batch_shape = [len(tensors)] + list(tensors[0].shape[:-2]) + list(max_size)
            batched_imgs = tensors[0].new_full(batch_shape, pad_value) # 新建一个target_batch shape tensor
            for img, pad_img in zip(tensors, batched_imgs):
                # pad_img: 值全为pad_value，把原来的tensor:img 赋值过去
                pad_img[..., : img.shape[-2], : img.shape[-1]].copy_(img)
            # 其实按照上面的方式也行，一个个pad
            # 因为分片过不连续，所以 contiguous()
