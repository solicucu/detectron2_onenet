...
def batched_nms(
    ...
    # 官方api,表示不对不同类之间作处理
    NMS, 对每一种类，先选score 最高的，再把同类的iou 大于阈值的全部移出，可以考虑先对
    idxs 的一个copy 集合化，依次对每个类进行处理
    """
    ...
    for id in torch.jit.annotate(List[int], torch.unique(idxs).cpu().tolist()):
        # .nonzero就是获取位置
        mask = (idxs == id).nonzero().view(-1) # 获取当前类别id的所有相关box 的位置
        keep = nms(boxes[mask], scores[mask], iou_threshold)
        result_mask[mask[keep]] = True # 能保留的设置为True

    keep = result_mask.nonzero().view(-1) # 获取为True 的所有位置
    # 根据分数的高低再进行排序一次
