...
def register_coco_instances(name, metadata, json_file, image_root):
    ...
    # 用lambda: 没有x，表示无需参数，直接执行后面的方法 load_coco_json
    DatasetCatalog.register(name, lambda: load_coco_json(json_file, image_root, name))

    # 2. Optionally, add metadata about this dataset,
    # since they might be useful in evaluation, visualization or logging
    #初次调用，没有，就会创建一个空的Metadata类返回
...
if __name__ == "__main__":
    ...
    assert sys.argv[3] in DatasetCatalog.list() # 3 is dataset_name
    meta = MetadataCatalog.get(sys.argv[3]) # 获取元数类，空

    dicts = load_coco_json(sys.argv[1], sys.argv[2], sys.argv[3]) # 加载数据
    """
    item in dicts:
    {
    "file_name":图片完全路径
