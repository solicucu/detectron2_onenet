...
def filter_images_with_only_crowd_annotations(dataset_dicts):
    ...
    def valid(anns):
        for ann in anns: # 为空，就return False
...
def print_instances_class_histogram(dataset_dicts, class_names):
    ...
    total_num_instances = sum(data[1::2])#  data[1::2] a[i:j:step]表示从index = 1 开始切片，直到末尾，step = 2
...
def get_detection_dataset_dicts(
    ...
    # 从DatasetCatalog.get() 获取数据集字典
    # 2021.2.22 
    # 所以在哪里已经注册了，这里就可以直接获取了
    # each DatasetCatalog.get() return a list[dict]
    dataset_dicts = [DatasetCatalog.get(dataset_name) for dataset_name in dataset_names]
    # 返回的dataset_dicts, 每个item的format:
    """
    item in dicts:
    {
    "file_name":图片完全路径
    ...
    "anotations":[{
        ...
        "segmentation"：语义分割信息
        "keypoints"：关键点信息
        "bbox_mode"：xywh_abs 1
        "category_id":类别id(可能是已经映射过的，连续的)
        },....]
    }
    """
    # 2021.2.23
    ...
    # 把多个列表链接，采用惰性的方式读取，也就是yeild ...， 需要再读取的模式
...
def _train_loader_from_config(cfg, *, mapper=None, dataset=None, sampler=None):
    if dataset is None:
        dataset = get_detection_dataset_dicts(
            ...
            filter_empty=cfg.DATALOADER.FILTER_EMPTY_ANNOTATIONS, # defualt:True 过滤掉没有标注的图片
            min_keypoints=cfg.MODEL.ROI_KEYPOINT_HEAD.MIN_KEYPOINTS_PER_IMAGE
            if cfg.MODEL.KEYPOINT_ON #default:False
            else 0,# cfg.MODEL.LOAD_PROPOSALS : default:False
            proposal_files=cfg.DATASETS.PROPOSAL_FILES_TRAIN if cfg.MODEL.LOAD_PROPOSALS else None,
        )

    if mapper is None:
        # DatasetMapper(cfg=, is_train=)
        mapper = DatasetMapper(cfg, True)
        # when mapper is call, 读取图片并映射dataset_dict to 一个包含 gt_boxes, gt_classes 的instance
        """ mapper(dataset_dict)
        dataset_dict
        +
        "image"
        "instances"： intances.image_size 是transform 后的大小
        -
        "segmentation"
        "keypoints"
        "annotations"
    ...
    if sampler is None:
        ...
        if sampler_name == "TrainingSampler":
            # an iterator return with indices:shuffle(range(1,len(dataset))) with start at rank and step is world_size
    ...
    return {
        ...
        # 每个batch 的图片 aspect_ratio 要兼容
        "aspect_ratio_grouping": cfg.DATALOADER.ASPECT_RATIO_GROUPING, # defualt: True,
        "num_workers": cfg.DATALOADER.NUM_WORKERS, # 4
    }

"""
build_detection_trian_loader(cfg)-> 会先调用修饰器configurable
"""
...
def build_detection_train_loader(
    ...
    if isinstance(dataset, list):
        # 转化为torch.utils.data.Dataset
        dataset = DatasetFromList(dataset, copy=False)
    if mapper is not None:
        # Datase wrapped with mapper
