...
class _DatasetCatalog(UserDict):# 这个类别就是一个字典
    """
    ...
    def get(self, name):
        ...
        # 返回f() 函数的调用，f() 是一个无参函数，返回一个list[dict]
        return f()
...
class _MetadataCatalog(UserDict):
    ...
    It's like global variables, so don't abuse it. # 滥用
    ...
    def get(self, name):
        ...
        """
        assert len(name)
        r = super().get(name, None)# 调用超类的get， 为什么不像Category 直接用self[name]访问
        if r is None:
