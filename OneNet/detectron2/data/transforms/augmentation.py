...
# aug: a class of Augmentation
# aug_input: a class of AugInput
def _get_aug_input_args(aug, aug_input) -> List[Any]:
    ...
    if aug.input_args is None:
        ...
        # signature 签名 
        # inspect.signature(function).parameters 获取function 的参数, 是一个orderdict
        # 所以用.items() 访问 
        ...
        # 长度是1那就铁定是图片，如果非1，按照下面执行
        if len(prms) == 1:
            names = ("image",)
        else:
            # 依次获取augmentation.get_transform 的参数，注意参数要是 Any fixed-length positional arguments
            names = []
...
class Augmentation:
    ...
    def get_transform(self, *args) -> Transform:
        ...
        Examples:
        ::
            class MyAug: # 应该要继承Augmentation类的
        ...
        Notes:
            ...
            input data.(input data 应该是指 AugIput) In detectron2 we use the following convention:
            在detectron2 中，get_transform 只支持下面3种属性(参数)
    ...
    def __call__(self, aug_input) -> Transform:
        ...
        # 获取 真正的transform （fvcore.transforms.transform.Transform 类型）
        # 获取 transform
...
def _transform_to_aug(tfm_or_aug):
    ...
    if isinstance(tfm_or_aug, Augmentation):
        ...
        # 如果是Transfrom 类型，转化为Augmentation 类型
...
class AugmentationList(Augmentation):
    ...
    def __call__(self, aug_input) -> Transform:
        ...
        for x in self.augs:
            ...
            # tfm is a instance of Transform
            tfms.append(tfm)
        # 返回Transorm 是可以用于处理User 需要处理的别的东西
...
class AugInput:
    ...
    * The input must have a ``transform(tfm: Transform) -> None`` method which
    ...
    # 这个方法在AugInput 作为 调用Augmention() 时的参数会被调用，see in Augmention.__call__
