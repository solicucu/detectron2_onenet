...
def configurable(init_func=None, *, from_config=None):
    """装饰一个函数或者一个类的初始化方法，使得该函数或方法可以用cfg对象作为参数被调用，同时使用from_config 
    函数来解析cfg成为需要的参数
    ...
    ::
        ...
        # or @configurable(from_config=translate)
    ...
    if init_func is not None:
        assert (
            inspect.isfunction(init_func) # 判断是一个函数
            and from_config is None # 不能同时部位空
            and init_func.__name__ == "__init__" # 要为类的初始化函数才合法
