...
# deformable convoutional network https://zhuanlan.zhihu.com/p/52476083
class DCNDeconvLayer(nn.Module):
    ...
    def __init__(
        ...
        # modulate(调节，调整) deform(使变形)
        self.deform_modulated = modulate_deform 
        if modulate_deform:
            deform_conv_op = ModulatedDeformConv # forward need (offset, mask)
            # offset channels are 2 or 3 (if with modulated) * kernel_size * kernel_size
            offset_channels = 27 
            # k * k * 3, (3 是每个卷积核的位移量(x,y,mask))偏移channel, 用于deformable convolution
        else:
            deform_conv_op = DeformConv #forward need (offset)
            offset_channels = 18 #(3 * 3 * 2)
...
# normal convolution + deconv 与 deformable convolution + deconv 作对比
